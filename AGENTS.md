...
This repository provides a Codex MCP plugin for read-only X/Twitter research
through xAI's Responses API `x_search` tool.
...
- Do not print, log, commit, or ask the user to paste access tokens, refresh
  ...
  `python3 scripts/x_search_auth.py login --manual-paste` terminal prompt,
...
- OAuth state belongs under `~/.x-search-codex/auth.json` by default, or under
  `X_SEARCH_CODEX_HOME` when explicitly set.
...
`skills/x-search-codex/SKILL.md` in the same change. Keep docs operational:
...
plugins/x-search-codex/.codex-plugin/plugin.json
plugins/x-search-codex/.mcp.json
plugins/x-search-codex/skills/...
plugins/x-search-codex/scripts/...
```

The root files are the development source of truth. The `plugins/x-search-codex`
...
local marketplace registration and `codex plugin add x-search-codex@x-search-codex`.
